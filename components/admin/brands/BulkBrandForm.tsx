...
import { ArrowLeft, Layers, List, CheckCircle2 } from 'lucide-react';
import { Card } from '@/components/ui/Card';
import { handleError } from '@/lib/errorHandler';
import { useCatalog } from '@/components/providers/CatalogContext';

export default function BulkBrandForm() {
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            await api.post('/admin/brands/bulk', { names: rawNames });
            await refreshCatalog();
            toast.success('Marcas creadas correctamente');
            router.push('/brands');
        } catch (error) {
            toast.error(handleError(error, 'Bulk Brand Create'));
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-5xl mx-auto p-6">
            <div className="flex items-center gap-4 mb-8">
                <button onClick={() => router.back()} className="p-2 hover:bg-white rounded-full transition shadow-sm"><ArrowLeft className="w-5 h-5" /></button>
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><Layers className="w-6 h-6 text-blue-600" /> Carga Masiva de Marcas</h1>
                ...
                <div className="space-y-6 lg:sticky lg:top-6 h-fit">
                    <Card className="p-0 overflow-hidden bg-slate-900 text-white border-0">
                        <div className="p-6 border-b border-slate-700 bg-slate-800/50">
                            <h3 className="font-bold text-lg flex items-center gap-2"><List className="w-5 h-5 text-green-400" /> Resumen</h3>
                        </div>
                    ...
                    <div className="bg-yellow-50 border border-yellow-100 rounded-xl p-4 text-xs text-yellow-800 flex gap-2">
                        <CheckCircle2 className="w-4 h-4 shrink-0" />
                        <span>Se omitirán nombres que ya existan en la base de datos.</span>
                    </div>
                </div>
            </div>
        </div>
